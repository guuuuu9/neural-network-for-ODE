...
if __name__ == '__main__':
    ...
    def plot_results(time, time_pred, X_pred, X):
        time = time.detach().numpy()
        time_pred = time_pred.detach().numpy()
        X_pred = X_pred.detach().numpy()
        plt.plot(time_pred, X_pred[:,0])
    ...
    pred = torch.arange(30, 35, 0.1)
    pred = torch.unsqueeze(pred, 1)
    t_pred = torch.cat((t, pred), 0)
    X_pred = ode_trained(X[0], t_pred, return_whole_sequence=True)
    plot_results(t, t_pred, X_pred, X)
    # writer.close()